            ...
            if (!this._throttled) {
                ...
                if (typeof( this._handleThrottleChanges ) == "function") {
                    ...
                    this._throttleTimeout = setTimeout(function () {
                            ...
                            if (typeof( this._handleThrottleChanges ) == "function") this._handleThrottleChanges(false, 0);
                        }.bind(this),
    ...
    var req = https.request({
        ...
        if (self._tokenRetries >= self._tokenMaxRetries) {
            ...
            while (0 < self._retryQueue.length) {
                ...
                rh = tuple[1];
                rh({
                    "status_code": "TOKEN_FAILURE",
                    "status_msg": "Failed to get access token. Contact Clarifai support."
                }, null);
            }
        }
        else {
...
Clarifai.prototype.tagImage = function (image, callback) {
    retry = function () {
        this.tagImage(image, callback);
    }.bind(this);

    this._tagImage(image, callback, retry);

};


Clarifai.prototype._tagImage = function (image, resultHandler, retry) {

    var self = this;
    fs.readFile(image.filePath, function (err, data) {

        var type = image.dimensions.type;
        ...
        lwip.open(data, type)
            ...
            .then(function (form) {
                self._httpRequest(tagPath, form, image.guid, resultHandler, retry);
            })
            .catch(function (err) {
                resultHandler(err);
            });
...
Clarifai.prototype.feedbackAddTagsToDocids = function (docids, tags, localId, callback) {
    ...
    this._feedbackTagsDocids(docids, tags, localId, true, callback, function () {
        this.feedbackAddTagsToDocids(docids, tags, localId, callback);
    }.bind(this));
